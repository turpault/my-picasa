...
export function setupCrop(
  ...
  __(".btn-6x4", elem).on("click", () => {
    mode = "6x4";
    resize();
  });
  __(".btn-4x3", elem).on("click", () => {
    mode = "4x3";
    resize();
  });
  __(".btn-16x9", elem).on("click", () => {
    mode = "16x9";
    resize();
  });
  __(".btn-5x5", elem).on("click", () => {
    mode = "5x5";
    resize();
  });
  const ok = __(".btn-ok-crop", elem);
  const cancel = __(".btn-cancel-crop", elem);
  panZoomCtrl.events.on("pan", validate);
  panZoomCtrl.events.on("zoom", validate);

  __(".btn-orientation", elem).on("click", () => {
