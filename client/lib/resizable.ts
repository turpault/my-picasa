...
export function resizable(
  ...
  for (const gutterPos of pos.gutters) {
    ...
    parent.append(
      gutterElement
        .css({
          cursor: gutterPos.direction === "v" ? "ns-resize" : "ew-resize",
        })
        .on("mousedown", () => {
  ...
  function resize(ev: MouseEvent | TouchEvent) {
    ev.preventDefault();
    const x = (ev as MouseEvent).x || (ev as TouchEvent).touches[0].clientX;
    const y = (ev as MouseEvent).y || (ev as TouchEvent).touches[0].clientY;
    if (!movingGutter) return;
    const gutterPos = movingGutter.getData()[0];
    if (!gutterPos) return;
    if (gutterPos.direction === "v") {
      // recalculating left and right weights
      const totalWidth = gutterPos.leftCell.width + gutterPos.rightCell.width;
      const left = gutterPos.leftCell.left;
      gutterPos.leftCell.weight = (x - left) / totalWidth;
      gutterPos.rightCell.weight = 1 - gutterPos.leftCell.weight;
    } else {
      const totalHeight =
        gutterPos.leftCell.height + gutterPos.rightCell.height;
      const top = gutterPos.leftCell.top;
      gutterPos.leftCell.weight = (y - top) / totalHeight;
      gutterPos.rightCell.weight = 1 - gutterPos.leftCell.weight;
    }
